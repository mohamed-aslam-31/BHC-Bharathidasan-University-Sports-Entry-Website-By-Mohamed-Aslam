...
export default function DraftPage() {
  ...
  const [confirmDelete, setConfirmDelete]     = useState(null); // draft id
  ...
  const handleResume = (draft) => {
    navigate('/students/new', { state: { draftId: draft.id } });
  };
        ...
        <div className="space-y-3">
          {drafts.map((draft) => {
            ...
            return (
              <div
                key={draft.id}
                className="card p-5 flex flex-col gap-3 hover:border-blue-200 dark:hover:border-blue-700 transition-colors"
              >
